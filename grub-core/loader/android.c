...
static grub_err_t
grub_e820_add_region (
    struct grub_e820_mmap *e820_map,
    int *e820_num,
    grub_uint64_t start,
    grub_uint64_t size,
    grub_uint32_t type
    ...
    void *data)
{
    ...
    if (grub_e820_add_region (ctx->params->e820_map, &ctx->e820_num,addr, size, type))
        return 1;
    return 0;
}
...
{
    cmd_android = grub_register_command ("android",
    grub_cmd_android,
    0,
    N_("Load android.")
    );
